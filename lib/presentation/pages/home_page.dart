...
class _HomePageState extends State<HomePage> {
  // --- YOUR ORIGINAL DATA & LOGIC (UNCHANGED) ---
  String selectedCategory = "Cappuccino";
  int _currentTabIndex = 0; // Added for navigation
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // Change background color depending on active screen
      backgroundColor: _currentTabIndex == 0 ? const Color(0xFFF9F9F9) : Colors.black,
      body: SingleChildScrollView(
        // Switch between your Home grid and other screens
        child: _currentTabIndex == 0
            ? homeBody
            : _currentTabIndex == 1
            ? const FavoriteScreen()
            : _buildPlaceholder(_currentTabIndex),
      ),
      bottomNavigationBar: _buildBottomNav(),
    );
  }
  ...
  Widget _buildPlaceholder(int index) {
    String title = index == 2 ? "Cart" : "Notifications";
    return Container(
      height: 500,
              ...
              children: [
                ...
                Row(
                  children: [
                    const Text(
                      "West, Balurghat ",
                      style: TextStyle(color: Color(0xFFDDDDDD), fontWeight: FontWeight.w600, fontSize: 14, fontFamily: 'Sora'),
                    ),
                    Image.asset("asset/icons/Arrow - Down 2.png", width: 14, height: 14),
                  ],
                ),
